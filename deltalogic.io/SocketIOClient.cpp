...
void SocketIOClient::eventHandler(int index) {
	...
	//if (ioDebug) Serial.println("[Server]:OK");//+rcvdmsg); //Can be used for debugging
	  ...
	  case '4':
          ...
          data = rcvdmsg.substring(rcvdmsg.indexOf("\",") + 3, rcvdmsg.length () - 2);
			    ...
			    for (uint8_t i = 0; i < onIndex; i++) {
				    if (id == onId[i]) {
					    if (ioDebug) //Serial.println("[App Event] Found handler = " + String(i));
					    (*onFunction[i])(data);
				    }
			    }
			    break;
...
void SocketIOClient::emit(String id, String data) {
	...
	if (ioDebug) Serial.println("[emit] " + data);
	int header[10];
	header[0] = 0x81;
